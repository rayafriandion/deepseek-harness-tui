...
import { displayWidth, truncateWidth, timeString, toolSummary, roughTokens } from './util.js'
...
export class App {
  ...
  render() {
    ...
    const left = ' ' + (this.model || 'model') + ' · ' + tokText
    screen.text(leftX, statusRow, truncateWidth(left, Math.max(0, Math.floor(cols * 0.42))), statusStyle)
    const readings = []
    ...
    if (right && metricRight > metricLeft) {
      ...
      screen.text(Math.max(metricLeft, metricRight - displayWidth(clipped)), statusRow, clipped, makeStyle({ fg: t.secondary, bg: t.background }))
    }
  ...
  _paintSettings(screen, cols, rows) {
    ...
    for (let offset = 0; offset < items.length; offset++) {
      ...
      const value = confirming ? 'Y confirm · N cancel' : this.settingsEditing === i ? draft + '█' : item.value
  ...
  _paintHelp(screen, cols, rows) {
    ...
    const rows2 = [
      ...
      ['Ctrl+N', 'new session'],
      ['Ctrl+D', 'delete selected session in Manage sessions'],
